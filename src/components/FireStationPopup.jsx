import styles from "./FireStationPopup.module.css";

const FireStationPopup = (props) => {
  const fireStation = props.fireStation;
  const handleFindRoute = props.handleFindRoute;
  ...
  const buildAddress = (fireStation) => {
    ...
    address +=
      fireStation["addr:housenumber"] + " " + fireStation["addr:street"];
  ...
  return (
    <div>
      ...
      {!fireStation.official_name && fireStation.name && (
        <p>Name: {fireStation.name}</p>
      )}
      {fireStation["addr:housenumber"] && fireStation["addr:street"] && (
        <p>Address: {buildAddress(fireStation)}</p>
      )}
      ...
      <div className={styles.routeButtonContainer}>
        <button className={styles.routeButton} onClick={handleFindRoute}>
          Find route to selected fire
        </button>
      </div>
    </div>
  );
